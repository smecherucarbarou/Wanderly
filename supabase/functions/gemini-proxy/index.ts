...
function truncateForLog(value: string, maxLength = 1200): string {
  ...
  return safeValue.length > maxLength
    ? `${safeValue.slice(0, maxLength)}...<truncated>`
    : safeValue;
}
...
function sanitizeGeminiResponse(responseText: string): string {
  try {
    ...
    if (Array.isArray(parsed.candidates)) {
      sanitized.candidates = parsed.candidates;
    }
    if (isRecord(parsed.usageMetadata)) {
      sanitized.usageMetadata = parsed.usageMetadata;
    }
    if (typeof parsed.modelVersion === "string") {
      sanitized.modelVersion = parsed.modelVersion;
    }
...
function extractSystemInstructionText(
...
function withServerSystemInstruction(
  payload: Record<string, unknown>,
...
function dailyLimit(envName: string, fallback: number): number {
  const parsed = Number.parseInt(Deno.env.get(envName) ?? "", 10);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : fallback;
}

function normalizeAiQuotaResult(data: unknown): AiQuotaResult {
  ...
  return {
    ...
    is_plus: source.is_plus === true,
    used: Number.isFinite(Number(source.used)) ? Number(source.used) : 0,
    limit: Number.isFinite(Number(source.limit)) ? Number(source.limit) : 0,
    remaining: Number.isFinite(Number(source.remaining))
      ? Number(source.remaining)
      : 0,
    reset_date: typeof source.reset_date === "string" ? source.reset_date : "",
  };
}

async function consumeAiQuota(auth: AuthContext): Promise<AiQuotaResult> {
  if (!auth.userId) {
    return {
      allowed: false,
      is_plus: false,
  ...
  const { data, error } = await supabase.rpc("consume_ai_quota", {
    p_cost: 1,
    p_free_limit: dailyLimit("AI_FREE_DAILY_LIMIT", 5),
    p_plus_limit: dailyLimit("AI_PLUS_DAILY_LIMIT", 100),
  });
  if (error) {
    throw new Error("AI quota check failed");
  }
  return normalizeAiQuotaResult(data);
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    requestPhase = "validating_request_payload";
    const isGuideRequest = isWanderlyGuidePayload(payload);
    let guideRequest: GuideRequest | null = null;
    if (isGuideRequest) {
      const guideValidation = validateGuideRequest(payload);
      if (!guideValidation.ok) {
        return jsonResponse(req, {
          error: guideValidation.error,
          code: guideValidation.code,
          message: guideValidation.message,
        }, guideValidation.status);
      }
      guideRequest = guideValidation.value;
    } else if (!isValidGeminiPayload(payload)) {
      return jsonResponse(req, { error: "contents array is required" }, 400);
    }

    requestPhase = "checking_ai_quota";
    const quota = await consumeAiQuota(auth);
    if (!quota.allowed) {
      return jsonResponse(req, {
    ...
    const geminiPayload = guideRequest !== null
      ? buildGuideGeminiPayload(guideRequest)
      : payload as Record<string, unknown>;
    const hardenedPayload = withServerSystemInstruction(geminiPayload);
    ...
    if (!geminiResponse.ok && isModelFallbackStatus(geminiResponse.status)) {
      console.warn(
        `Primary model ${geminiModel} failed with ${geminiResponse.status}; retrying fallback model ${geminiFallbackModel}`,
      );
    ...
    if (!geminiResponse.ok) {
      ...
      console.error("Gemini upstream request failed", {
        ...
        model: geminiResponse.url.includes(geminiFallbackModel)
          ? geminiFallbackModel
          : geminiModel,
      });
      return jsonResponse(req, {
    ...
    if (guideRequest !== null) {
      ...
      try {
        message = sanitizeGuideAssistantMessage(
      ...
      if (message === null) {
        return jsonResponse(
          req,
          { error: "Assistant response unavailable" },
          502,
        );
      }
      return jsonResponse(req, { message, quota });
    }
